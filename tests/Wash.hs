...
testWashSaleRule = testGroup "washSaleRule"
    [
    --   testCase "open" $
    --   flip evalStateT newWashState $ do
...
type WashState a = (WashHistory a, [a], [a])

newWashState :: WashState a
newWashState = (newWashHistory, [], [])

wash :: (Transactional a, Show a, Eq a)
     => a -> StateT (WashState a) IO ([a] , [a])
wash pl = do
    zoom _1 $ do
        lift $ putStrLn $ render $ text "pl      = " <> text (showPretty pl)
        hs <- use history
        lift $ putStrLn $ render $ text "before  = " <> renderList (text . ppShow) hs
        raw <- hoist (pure . runIdentity) $ newEntry pl
        lift $ putStrLn $ render $ text "raw     = " <> text (ppShow raw)
        let actions = findMatches hs raw
        lift $ putStrLn $ render $ text "actions = " <> renderList (text . ppShow) actions
        (ent, ents) <- hoist (pure . runIdentity) $ evalActions raw actions
        lift $ putStrLn $ render $ text "ent     = " <> text (ppShow ent)
        lift $ putStrLn $ render $ text "ents    = " <> renderList (text . ppShow) ents
        hs' <- use history
        lift $ putStrLn $ render $ text "after   = " <> renderList (text . ppShow) hs'

    zoom (zipped _2 _3) $ do
