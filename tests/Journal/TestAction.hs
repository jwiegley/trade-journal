...
bought :: MonadIO m => Annotated Lot -> StateT (Positions a) m ()
bought b = entries <>= [Action . Buy <$> b]

sold :: MonadIO m => Annotated Lot -> StateT (Positions a) m ()
...
close ::
  ...
  Amount 6 ->
  StateT (Positions a) m ()
close n b pl = closeWashed n b pl mempty
