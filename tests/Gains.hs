...
testGainsKeeper = testGroup "gainsKeeper"
    ...
    , testCase "handleFees opening position" $
      handleFees @API.Transaction
          0.81 [ (True, 100 @@ 1000.00 $$$ 0.0) ]
          @?= [ (True, 100 @@ 1000.81 $$$ 0.0) ]

    , testCase "handleFees closing single position" $
      handleFees @API.Transaction
          0.81 [ (False, (-100) @@ 1000.81 $$$ 199.19) ]
          @?= [ (False, (-100) @@ 1000.81 $$$ 200.00) ]

    , testCase "handleFees closing multiple positions 1" $
      handleFees @API.Transaction
          0.81 [ (False, (-100) @@ 1000.81 $$$ (-100.00))
               , (False, (-100) @@ 1000.81 $$$ (-100.00))
               ]
          @?= [ (False, (-100) @@ 1000.81 $$$ (-100.00 + 0.40 + 0.01))
              , (False, (-100) @@ 1000.81 $$$ (-100.00 + 0.40))
              ]

    , testCase "handleFees closing multiple positions 2" $
      handleFees @API.Transaction
          0.83 [ (False, (-10.00) @@ 19740.50 $$$ (-100.00))
               , (False, (-10.00) @@ 19707.90 $$$ (-100.00))
               ]
          @?= [ (False, (-10.00) @@ 19740.50 $$$ (-100.00 + 0.41 + 0.01))
              , (False, (-10.00) @@ 19707.90 $$$ (-100.00 + 0.41))
              ]

    , testCase "handleFees closing multiple positions 3" $
      handleFees @API.Transaction
          0.47 [ (False, (-689.00) @@ 11864.3044 $$$ (-34.87))
               , (False, (-300.00) @@ 5165.97 $$$ (-15.09))
               ]
          @?= [ (False, (-689.00) @@ 11864.3044 $$$ (-34.54))
              , (False, (-300.00) @@ 5165.97 $$$ (-14.95))
              ]
